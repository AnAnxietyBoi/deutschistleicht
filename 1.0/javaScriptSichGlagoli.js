...
function localStoragee()
{
	...
	if ((typeof(Storage) !== "undefined") && !isIE) //če lahko brskalnik ima local storage
	{
		...
		{
			p = 0;
			pravilni = JSON.parse(localStorage.getItem(datum + "_pravilniSich")) || [[0, 0], [0, 0]]; //json.parse iz stringa naredi array
			steviloGlagolov = JSON.parse(localStorage.getItem(datum + "_steviloGlagolovSich")) || [[0, 0], [0, 0]];
			napacniGlagoli = JSON.parse(localStorage.getItem(datum + "_napacniGlagoliSich")) || [[[], []], [[], []]];
		}
		else
		{
			localStorage.setItem(datum + "_pravilniSich", JSON.stringify(pravilni)); //array se spremeni v string
			localStorage.setItem(datum + "_steviloGlagolovSich", JSON.stringify(steviloGlagolov));
					...
					else //NAROBE
					{
						...
						if (document.getElementById("input" + o).value != "") //če ni odgovor prazen
						{
							...
							for (var z = 0; z <= vsiGlagoli[AD][o][i[AD]].length - 1; z++) //za vsako besedo v arrayu glagola
							{
								...
								if (obeB[1] !== undefined) //če je 1 stolpec
								{
								...
								for (var u = 0 + (o * 5); u <= odgovor.length - 1 - ((clen + 1) * Math.abs(o - 1)); u++) //za vsako črko odgovora, -2 zaradi presledka, -clen ker je na koncu
								{
