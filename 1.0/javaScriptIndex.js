...
function localStoragee()
{
	...
	if ((typeof(Storage) !== "undefined") && !isIE) //če lahko brskalnik ima local storage
	{
		...
		{
			p = 0;
			pravilni = JSON.parse(localStorage.getItem(datum + "_pravilniNavadni")) || [0, 0, 0, 0, 0]; //json.parse iz stringa naredi array
			steviloGlagolov = JSON.parse(localStorage.getItem(datum + "_steviloGlagolovNavadni")) || [0, 0, 0, 0, 0];
		...
		else
		{
			...
			localStorage.setItem(datum + "_pravilniNavadni", JSON.stringify(pravilni)); //array se spremeni v string
			localStorage.setItem(datum + "_steviloGlagolovNavadni", JSON.stringify(steviloGlagolov));
