...
function localStoragee()
{
	if (typeof(Storage) !== "undefined") //če lahko brskalnik ima local storage
	{
		...
		{
			p = 0;
			pravilni = JSON.parse(localStorage.getItem("pravilniSich_" + datum)) || [[0, 0], [0, 0]]; //json.parse iz stringa naredi array
			steviloGlagolov = JSON.parse(localStorage.getItem("steviloGlagolovSich_" + datum)) || [[0, 0], [0, 0]];
			napacniGlagoli = JSON.parse(localStorage.getItem("napacniGlagoliSich_" + datum)) || [[[], []], [[], []]];
		}
		else
		{
			localStorage.setItem("pravilniSich_" + datum, JSON.stringify(pravilni)); //array se spremeni v string
			localStorage.setItem("steviloGlagolovSich_" + datum, JSON.stringify(steviloGlagolov));
			localStorage.setItem("napacniGlagoliSich_" + datum, JSON.stringify(napacniGlagoli));
		}
	}
}
...
function procentiPravilnih(o) //za izračun procentov pravilnih glagolov
{
	...
	if (fff.indexOf(1) > -1)
	{
		...
		var vsiPravilni = (pravilni[0].reduce((a, b) => a + b, 0)) + (pravilni[1].reduce((a, b) => a + b, 0));
		var vsiSteviloGlagolov = (steviloGlagolov[0].reduce((a, b) => a + b, 0)) + (steviloGlagolov[1].reduce((a, b) => a + b, 0));
			...
			for (var o = 0; o <= 1; o++)
			{
				...
				if (fff[o] == 1)
				{
					...
					else //NAROBE
					{
						...
						if (o == 1)
						{
							napacniGlagoli[AD][o].push("sich " + vsiGlagoli[AD][o][i[AD]][0]); //doda glagol ki je bil napačen glede na stolpec	
						}
						else
						{
							napacniGlagoli[AD][o].push(vsiGlagoli[AD][o][i[AD]][0]); //doda glagol ki je bil napačen glede na stolpec
						}				
						...
						if (document.getElementById("input" + o).value != "") //če ni odgovor prazen
						{
							...
							for (var z = 0; z <= vsiGlagoli[AD][o][i[AD]].length - 1; z++) //za vsako besedo v arrayu glagola
							{
								...
								if ((o == 0) && (obeB[1] !== undefined)) //če je 5 stolpec
								{
									var nemObe = vsiGlagoli[AD][o][i[AD]][z].split(" ");
									clen = obeB[1].length;
									if (obeB[1] !== nemObe[1]) //če člen ni isti
									{
										for (var r = obeB[0].length + 1; r <= odgovor.length - 1; r++) //cel člen je nepravilen
										{
								...
								for (var u = 0; u <= odgovor.length - 2 - clen; u++) //za vsako črko odgovora, -2 zaradi presledka, -clen ker je na koncu
								{
						...
						var sichh = ["", "sich "];
						document.getElementById("pravilniGlagol" + o).innerHTML = "Pravilni glagol: " + "<span>" + sichh[o] + vsiGlagoli[AD][o][i[AD]][0] + "</span>";
					}
				}
				procentiPravilnih(o);
				localStoragee();
...
function reset()
{
	...
	localStorage.removeItem("pravilniSich_" + datum);
	localStorage.removeItem("steviloGlagolovSich_" + datum);
	pravilni = [[0, 0], [0, 0]];
