...
function localStoragee()
{
	if (typeof(Storage) !== "undefined") //če lahko brskalnik ima local storage
	{
		...
		{
			p = 0;
			pravilni = JSON.parse(localStorage.getItem("pravilniNavadni_" + datum)) || [0, 0, 0, 0, 0]; //json.parse iz stringa naredi array
			steviloGlagolov = JSON.parse(localStorage.getItem("steviloGlagolovNavadni_" + datum)) || [0, 0, 0, 0, 0];
			napacniGlagoli = JSON.parse(localStorage.getItem("napacniGlagoliNavadni_" + datum)) || [[], [], [], [], []];
		}
		else
		{
			localStorage.setItem("pravilniNavadni_" + datum, JSON.stringify(pravilni)); //array se spremeni v string
			localStorage.setItem("steviloGlagolovNavadni_" + datum, JSON.stringify(steviloGlagolov));
			localStorage.setItem("napacniGlagoliNavadni_" + datum, JSON.stringify(napacniGlagoli));
		}
	}
}
...
{	
	...
	localStorage.removeItem("pravilniNavadni_" + datum);
	localStorage.removeItem("steviloGlagolovNavadni_" + datum);
	pravilni = [0, 0, 0, 0, 0];
